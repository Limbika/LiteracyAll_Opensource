...
public class CustomListAdapter extends RecyclerView.Adapter<CustomListAdapter.CustomListViewHolder> {
    ...
    SendSequenceList sendSequenceList;
    ...
    public void onBindViewHolder(CustomListViewHolder holder, int position) {
        ...
        imageProcessing.setImageWith_loaderRowItem(holder.ivSqGridItem, customList.imageName);
    ...
    public class CustomListViewHolder extends RecyclerView.ViewHolder {
        @Bind(R.id.ivSqGridItem)
        ImageView ivSqGridItem;
                ...
                case DragEvent.ACTION_DROP:
                    ...
                    if (v.getId() == R.id.cardView || v.getId() == R.id.textEmptyList) {
                        ...
                        if (v.getId() == R.id.textEmptyList) {
                            target = (RecyclerView) v.getRootView().findViewById(R.id.recyclerPendingList);
                        } else {
                        ...
                        if (source.getId() == R.id.recyclerPendingList && adapterSource.getItemCount() < 1) {
                            mListener.setEmptyList(true);
                        }
                        ...
                        if (target == (RecyclerView) v.getRootView().findViewById(R.id.recyclerPendingList))
                            sendSequenceList.onDrop(customListTarget);
                        else
                            sendSequenceList.onDrop(customListSource);
                    }
    ...
    public interface SendSequenceList {
        void onDrop(List<CustomList> customListTarget);
    }

    public void setSendSequenceListListener(SendSequenceList sendSequenceList) {
        this.sendSequenceList = sendSequenceList;
    }
}
